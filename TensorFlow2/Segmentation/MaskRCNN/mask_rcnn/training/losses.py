...
def _giou_loss(y_true, t_pred, weights):
    y_pred = tf.cast(y_pred, tf.float32)
    y_true = y_pred = tf.cast(y_true, ypred.dtype)
    giou = tf.squeeze(_calculate_giou(y_pred, y_true))
    giou_loss = 1. - giou
    # weights to filter out relevant
    giou_loss = tf.math.multiply(giou_loss, weights)
    giou_loss = tf.math.reduce_sum(giou_loss)
